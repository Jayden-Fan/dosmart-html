...
(function ($) {
    ...
    function _58(_59) {
        ...
        _5f(dc.header1, _5b.frozenColumns, true,_59.id,_5b);
        _5f(dc.header2, _5b.columns, false,_59.id,_5b);
        _60();
        ...
        if (_5b.toolbar) {
            if ($.isArray(_5b.toolbar)) {
                ...
                var tb = $("<div class=\"erp-toolbar datagrid-toolbar\"></div>").prependTo(_5c);
                for (var i = 0; i < _5b.toolbar.length; i++) {
                    var btn = _5b.toolbar[i];
                    if (btn == "-") {
                        //$("<td><div class=\"datagrid-btn-separator\"></div></td>").appendTo(tr);
                    } else {
                        var _65 = $("<a href=\"javascript:void(0)\" class=\"l-btn l-btn-small l-btn-plain\"></a>").appendTo(tb);
                        _65.attr("data-options","iconCls:'" + btn.iconCls + "',plain:true");
                        if(btn.iconCls == 'iconfont-dot-more'){
                            _65.addClass("easyui-linkbutton dot-more");
                            _65.attr('id','dounion_dot_more');
                            if(btn.menuId){
                                _65.attr('menuId',btn.menuId);
                                _65[0].onclick = eval(function(){
                                    var _menuId = $(this).attr('menuId');
                                    if($('#' + _menuId).length > 0) {
                            ...
                            if(btn.menuId){
                                _65.addClass("easyui-menubutton m-btn m-btn-small");
                                _65.attr('menuId',btn.menuId);
                                _65[0].onclick = eval(function(){
                                    var _menuId = $(this).attr('menuId');
                                    if($('#' + _menuId).length > 0){
                                ...
                                if(btn.handler){
                                    _65.addClass("easyui-linkbutton");
                                    _65[0].onclick = eval(btn.handler || function(){});
                                }
                            }
                        }
                        var _span = $("<span class=\"l-btn-left l-btn-icon-left\"></span>");
                        var _span1 = $("<span class=\"l-btn-text\"></span>");
                        if(btn.text){
                            _span1.html(btn.text);
                        }else{
                            _span1.html('&nbsp;');
                            _span1.addClass('l-btn-empty');
                        }
                        var _span2 = $("<span class=\"l-btn-icon\">&nbsp;</span>");
                        if(btn.iconCls){
                            _span2.addClass(btn.iconCls);
                        }
                        _span.append(_span1);
                        _span.append(_span2);
                        if(btn.menuId){
                            $('<span class="m-btn-downarrow"></span><span class="m-btn-line"></span>').appendTo(_span);
                        }
                        _65.append(_span);
        ...
        if (_5b.pagination) {
            ...
            _62.pagination({
                ...
                themes:_5b.themes,
                topHtml:_5b.topHtml,
                hideLinks:_5b.hideLinks,
                onSelectPage: function (_64, _65,_18f) {
        ...
        function _5f(_66, _67, _68,datagridId,_options) {
            if (!_67) {
                return;
            }
            $(_66).show();
                ...
                for (var j = 0; j < _6c.length; j++) {
                    ...
                    if (col.checkbox) {
                        ...
                        if (col.field) {
                            ...
                            if(!col.menuId){
                                var _fieldId = col.field;
                                var _mId = _fieldId.replace(/[\.|\s]/g, "_");
                                if($('#' + datagridId + '_field_menuId_' + _mId).length > 0){
                                    col.menuId = datagridId + '_field_menuId_' + _mId;
                                }
                            }
                            if(col.menuId){
                                var _queryFieldName = col.field;
                                if(col.filterField){
                                    _queryFieldName = col.filterField;
                                }
                                var _filterCss = '';
                                if(_options.queryParams[_queryFieldName]){
                                    _filterCss = ' datagrid-downfilter-icon ';
                                }
                                var _menu = $('<i class="iconfont-sj-arrd datagrid-downmenu-icon ' + _filterCss + '" field="' + col.field + '" menuId="' + col.menuId + '" datagridId="' + datagridId + '">&nbsp;</i>');
                                if(typeof eval(col.menuId) == 'function'){
                                    $(_menu).bind('click',eval(col.menuId));
                                }else if($('#' + col.menuId).length > 0){
                                    $(_menu).bind('click',function(e){
                                        var _menuId = $(this).attr('menuId');
                                        $('#' + _menuId).menu({onHide:function(){
                                            _menu.removeClass('curr');
                                        }});
                                        $(this).addClass('curr');

                                        var grid = $('#' + $(this).attr('datagridId'));
                                        var _sortName = grid.datagrid('options').sortName;
                                        var _sortOrder = grid.datagrid('options').sortOrder;
                                        var _field = $(this).attr('field');
                                        var _item = $('#' + _menuId).menu('findItem','按从A到Z排序');
                                        if(_item){
                                            var _clazz = '';
                                            if (_sortName == _field && 'asc' == _sortOrder) {
                                                _clazz = 'icon-sortaz';
                                            }
                                            $('#' + _menuId).menu('setIcon', {
                                                target: _item.target,
                                                iconCls: _clazz
                                            });
                                        }
                                        _item = $('#' + _menuId).menu('findItem','按从Z到A排序');
                                        if(_item){
                                            var _clazz = '';
                                            if (_sortName == _field && 'desc' == _sortOrder) {
                                                _clazz = 'icon-sortza';
                                            }
                                            $('#' + _menuId).menu('setIcon', {
                                                target: _item.target,
                                ...
                                if(!_options['filterTag']){
                                    _menu.hide();
                                }
                                td.append(_menu);
                                td.addClass("datagrid-downmenu");
                            }
                            if(col.field == 'linkid'){
                                var _html = $(col.title);
                                _html.find('a.iconfont-filter').bind('click',function(){
                                    var _options = $('#' + datagridId).datagrid('options');
                                    if(_options['filterTag']){
                                        _options['filterTag'] = false;
                                        $(this).removeClass('curr');
                                        $('i.datagrid-downmenu-icon[menuid]').each(function() {
                                            $(this).hide();
                                        });
                                        //清空各子查询，如果子查询有值，则执行一次 global_query_q 全局查询
                                        if(DatagridTools.clearFieldMenuSearch(datagridId)){
                                            if(window.doSearch && typeof(window.doSearch) == "function"){
                                                window.doSearch();
                                            }
                                        }
                                    }else{
                                        _options['filterTag'] = true;
                                        $(this).addClass('curr');
                                        $('i.datagrid-downmenu-icon[menuid]').each(function() {
                                            $(this).show();
                                        });
                                ...
                                if(window.doSearch && typeof(window.doSearch) == "function"){
                                    _html.find('a.iconfont-refresh').bind('click',function(){window.doSearch(true,false);});
                                }
                                _html.find('a.iconfont-set').bind('click',function(){DatagridTools.fun_show_dialog();});
                                td.find("span:first").append(_html);
                            }else{
    ...
    function _8d(e) {
        ...
        if (tt.parent().hasClass("datagrid-cell-check")) {
            ...
            if (td.length) {
                ...
                _8f.onClickCell.call(_8e, _90, _93,_getValue(_93,row));
            }
    ...
    function _98(e) {
        ...
        if (td.length) {
            ...
            _9a.onDblClickCell.call(_99, _9b, _9c,_getValue(_9c,row));
        }
    ...
    function _107(_108) {
        ...
        if (opts.idField) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                var _10d = _10a ? _getValue(opts.idField,row) : i;
    ...
    function _95(_127, _128, _129) {
        ...
        if (opts.idField) {
            ...
            _3(_12a.selectedRows, opts.idField, _getValue(opts.idField,row));
        }
    ...
    function _92(_13c, _13d, _13e) {
        ...
        if (opts.idField) {
            ...
            _3(_13f.checkedRows, opts.idField, _getValue(opts.idField,row));
        }
    ...
    function _147(_148, _149) {
        ...
        tr.find("div.datagrid-editable").each(function () {
            ...
            ed.actions.setValue(ed.target, _getValue(_14b,row));
        });
        _14c(_148, _149);
    ...
    function _14d(_14e, _14f, _150) {
        ...
        if (!_150) {
            ...
            tr.find("div.datagrid-editable").each(function () {
                ...
                //     _155[_156] = _158;
                // }
                if (_getValue(_156,row) !== _158) {
                    _setValue(_156,_158,row);
                    _154 = true;
                    _setValue(_156,_158,_155);
                }
            });
            if (_154) {
                if (_2(_153, row) == -1) {
    ...
    function _175(_176, _177) {
        ...
        _3(_178.selectedRows, opts.idField, _getValue(opts.idField,row));
        _3(_178.checkedRows, opts.idField,  _getValue(opts.idField,row));
    ...
    function _182(_183, _184) {
        ...
        for (var _187 in _184.row) {
            ...
            if (_getValue(_187,row) !== _getValue(_187,_184.row)) {
                _186 = true;
                break;
            }
        }
    ...
    function _setValue(name, val, data) {
    ...
    $.fn.datagrid.parseOptions = function (_1fd) {
        ...
        }, {pagination: "boolean", pageSize: "number", pageNumber: "number",themes:"string",topHtml:"string",hideLinks:"boolean"}, {
            multiSort: "boolean",
            remoteSort: "boolean",
            showHeader: "boolean",
            showFooter: "boolean"
            ...
            for (var i = 0; i < _212.length; i++) {
                ...
                if (col) {
                    ...
                    if (col.checkbox) {
                        ...
                        if (col.formatter) {
                            cc.push(col.formatter(_218, _215, _214,col));
                        } else {
                            if(col.field.indexOf('.')>-1 && _218 == undefined){
                                try{
                                    _218 = eval("_215['"+_217.replace(/\./g,"']['")+"']")
                                }catch(e){}
                            }
                            cc.push(_218);
                        }
