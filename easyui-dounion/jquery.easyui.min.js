...
$.parser={auto:true,onComplete:function(_b){
	//dounion 添加edatagrid
	},plugins:["draggable","droppable","resizable","pagination","tooltip","linkbutton","menu","menubutton","splitbutton","switchbutton","progressbar","tree","textbox","passwordbox","filebox","combo","combobox","combotree","combogrid","combotreegrid","tagbox","numberbox","validatebox","searchbox","spinner","numberspinner","timespinner","datetimespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","datalist","tabs","accordion","window","dialog","form","edatagrid"],parse:function(_c){
	//end
var aa=[];
...
return false;
	...
	//dounion combotree lines:true animate:true
$.fn.tree.defaults={url:null,method:"post",animate:true,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:true,dnd:false,data:null,queryParams:{},formatter:function(node){
	//end
return node.text;
...
return data;
	...
	//dounion panel collapse以后为panel-body添加自定义class
	,customClass:null
	//end
	//dounion panel 自定义箭头
	,customArrow:null
...
ep.css("cursor","default");
	...
	//dounion panel 自定义箭头鼠标滑过事件
	if(p.panel("options").customArrow){
		ep.bind("mouseover",function(){
			...
			if(p.panel("options").customArrow){
				$(".sub-menu-collect").animate({left:'220px'},function(){
					$(".sub-menu-collect").removeClass("sub-menu-show");
				});
			}
...
_3e0.center.panel("resize",_3e6.resizeC);
	...
	//dounion panel 自定义箭头收起动画
	if(p.panel("options").customArrow){
	$(".sub-menu-collect").animate({left:'0px'},function(){
		$(".sub-menu-collect").addClass("sub-menu-show");
	});
	}
	//end
...
function _3e5(){
	//dounion panel collapse之后为panel-body添加自定义class
	if(p.panel("options").customClass){
	_3e0[_3e2].parent().addClass(p.panel("options").customClass);
	_3e1.collapsedSize = _3e0[_3e2].parent().width();
	}
	//end
var cc=$(_3dd);
...
$("<div class=\"menu-icon\"></div>").addClass(_462.iconCls).appendTo(_462.target);
	...
	//dounion searchbox menu div为空报错
	if(typeof($(_463).data("menuitem"))=="undefined"){
	return;
	}
	//end
...
return _4f1=="true";
	...
	//dounion validate验证
		...
		email:{validator:function(_4eb){
			return /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(_4eb);
			},message:"Please enter a valid email address."},
		url:{validator:function(_4ec){
			return /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(_4ec);
			},message:"Please enter a valid URL."},
		length:{validator:function(_4ed,_4ee){
			var len=$.trim(_4ed).length;
			return len>=_4ee[0]&&len<=_4ee[1];
			},message:"Please enter a value between {0} and {1}."},
		remote:{validator:function(_4ef,_4f0){
			var data={};
			data[_4f0[1]]=_4ef;
			var _4f1=$.ajax({url:_4f0[0],dataType:"json",data:data,async:false,cache:false,type:"post"}).responseText;
			return _4f1=="true";
		...
		mobile: {validator: function (value, param) {
			return /^((\(\d{2,3}\))|(\d{3}\-))?13\d{9}$/.test(value);},message: 'Please enter a valid mobile.'},
		truckNo: {validator: function (value, param) {
			return /^([\u4E00-\u9FA5 0-9 A-Z a-z]*)$/.test(value);},message: '请输入正确的车号/车牌号.'},
		vehicleNo: {validator: function (value, param) {
			return /^[\u4E00-\u9FA5]([0-9A-Z]{6})|([0-9A-Z]{5}[\u4E00-\u9FA5]{1})$/.test(value);},message: 'Please enter a valid truckNo.'},
		intOrFloat:{validator : function(value) {
			return /^\d+(\.\d+)?$/i.test(value);},message : 'Please enter a valid intOrFloat.'},
		weightV:{validator : function(value) {//长度最长是12位，小数部分最多3位小数
			return /^(?=([0-9]{1,12}$|[0-9]{1,8}\.))(0|[1-9][0-9]*)(\.[0-9]{1,3})?$/i.test(value);},message : 'Please enter a valid weightR.'},
		digitV:{validator : function(value) {//数字，包括负数和整数
			return /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value);},message : 'Please enter a valid weightR.'},
		priceV:{validator : function(value) {//长度最长是12位，小数部分最多5位小数
			return /^(?=([0-9]{1,12}$|[0-9]{1,8}\.))(0|[1-9][0-9]*)(\.[0-9]{1,5})?$/i.test(value);},message : 'Please enter a valid weightR.'},
		amountV:{validator : function(value) {
			return /^(?=([0-9]{1,12}$|[0-9]{1,8}\.))(0|[1-9][0-9]*)(\.[0-9]{1,2})?$/i.test(value);},message : 'Please enter a valid amountV.'},
		qtyV:{validator : function(value) {
			return /^\d+$/i.test(value);},message : 'Please enter a valid qtyV.'},
		precisionfV:{validator : function(value) {
			return /^\d+(?:\.\d{1,4})?$/.test(value);},message: '请输入最多4位小数.'},
		precisionfvV:{validator : function(value) {
			return /^\d+(?:\.\d{1,5})?$/.test(value);},message: '请输入最多5位小数.'},
		precisionoV:{validator : function(value) {
			return /^\d+(?:\.\d{1,1})?$/.test(value);},message: '请输入最多1位小数.'},
		precisionotV:{validator : function(value) {
			return /^\d+(?:\.\d{1,2})?$/.test(value);},message: '请输入最多2位小数.'},
		precisionouV:{validator : function(value,param) {
			var reg = new RegExp("^\\d+(?:\\.\\d{1,"+param[0]+"})?$");
			return reg.test(value);},message: '最多输入{0}位小数.'},
		precisioniuV:{validator : function(value) {
			return /^[0-9]\d{0,11}$/.test(value);},message: '请输入最多12位整数.'},
		fourInt: {validator: function (value, param) {
			return /^[1-9]\d{3}$/.test(value);},message: '请输入4位整数.'},
		numberV: {validator: function (value, param) {
			return /^[1-9]\d{0,3}$/.test(value);},message: '请输入最多4位整数.'},
		dateTimeV: {validator: function (value, param) {
			return /^(\d{4})-(\d{2})-(\d{2})$/.test(value);},message: '日期格式不正确！.'},
		zip: {validator: function (value, param) {
		    return /^[1-9]\d{5}$/.test(value);},message: 'Please enter a valid zip.'}
	},
...
return false;
	...
	//dounion 重写load，新增getData方法
	function killNullProp(data){
	    if(data){
	        for(var i in data){
	...
	function load(target, data){
		...
		function _load(data){
			if(data == null) data = {};
			var form = $(target);
			form.data("row_data", killNullProp(data));
			var rr = form.find('input[type=radio],input[type=checkbox]');
			rr.each(function(){
				var f = $(this);
				if (_getValue(f.attr("name"), data)!=null && (_getValue(f.attr("name"), data) + "").indexOf(f.val()) != -1) {
					f.prop('checked',true);
				}
			});
			rr = form.find('input.combo,input.textbox-f,textarea.textbox-f,select,input.combo-text');
			rr.each(function(){
				var f = $(this);
				if(f.attr("textboxname") && !f.attr("comboName")){
					f["textbox"]('setValue', _getValue(f.attr("textboxname"), data));
				}else if (f.attr("type") != "checkbox"){
					f.val(_getValue(f.attr("name"), data));
				}
			});
			var cc = ['combobox','combotree','combogrid','datetimebox','datebox','combo'];
			rr = form.find('.combo-f');
			rr.each(function(){
				var f = $(this);
				for(var i=0; i<cc.length; i++){
					var type = cc[i];
					if (f.hasClass(type+'-f')){
						var name = f.attr("comboName");
						var names = name.split(".");
						if(names.length > 1) {
							names.pop();
							f.data("entity",_getValue(names.join("."), data));
						}
						if (f[type]('options').multiple){
							var combovalue = _getValue(name, data);
							if (combovalue != null && combovalue != '') {
								f[type]('setValues', combovalue);
							}
						} else {
							f[type]('setValue', _getValue(name, data));
						}
						break;
					}
				}
			});
			rr = form.find('.easyui-numberbox');
			rr.each(function(){
				var f = $(this);
				var v = _getValue(f.attr("numberboxName"), data);
				f.numberbox('setValue', v);
			});
	...
	function getData(target){
		...
		var form = $(target);
		var data = form.data("row_data");
		if(data == null) data = {};
		var rr = form.find('input[type=radio]:checked,input[type=checkbox]:checked');
		rr.each(function(){
			var f = $(this);
			_setValue(f.attr("name"), f.val(), data);
		});
		rr = form.find('.easyui-numberbox');
		rr.each(function(){
			var f = $(this);
			_setValue(f.attr("numberboxName"), f.numberbox('getValue'), data);
		});
		rr = form.find('input.combo,input.textbox-f,textarea,select,input.combo-text');
		rr.each(function(){
			var f = $(this);
			if(f.attr("textboxname") && !f.attr("comboName")){
				_setValue(f.attr("textboxname"), f["textbox"]("getValue"), data);
			}else{
				_setValue(f.attr("name"), f.val(), data);
			}
		});
		var cc = ['combobox','combotree','combogrid','datetimebox','datebox','combo','textbox'];
		rr = form.find('input.combo-f');
		rr.each(function(){
			var f = $(this);
			for(var i=0; i<cc.length; i++){
				var type = cc[i];
				if (f.hasClass(type+'-f')){
					...
					var val;
					if (f[type]('options').multiple){
						val = f[type]('getValues');
					} else {
						val = f[type]('getValue');
					}
					if(names.length > 1 && type.indexOf("combo") != -1) {
						val = val == "" ? null : f.data("entity");
						names.pop();
						name = names.join(".");
					}
					_setValue(name, val, data);
					break;
				}
			}
		});
...
function _670(_671,_672,_673){
	//dounion datagrid
	if($.data(_671,"datagrid")==null)return;
	//end
...
var ed=$.data(this,"datagrid.editor");
	//dounion datagrid editor出现时为editor赋值
	var _a=_771.split(".");
	var _5c5=row[_a[0]];
	for(var _k=1;_k<_a.length;_k++)if(_5c5!=null)_5c5=_5c5[_a[_k]];
	var _5c3=row[_a[0]];
	for(var _k=1;_k<_a.length-1;_k++)if(_5c3!=null)_5c3=_5c3[_a[_k]];
	ed.actions.setValue(ed.target,_5c5);
	_a.length>1&&$(ed.target).data("entity",_5c3);
	//end
...
var _77c=$(this).parent().attr("field");
	//dounion datagrid 可编辑datagrid editor编辑器点属性支持
	var _a=_77c.split(".");
	var _5c5=row[_a[0]];
	for(var _k=1;_k<_a.length;_k++)if(_5c5!=null)_5c5=_5c5[_a[_k]];
	_5c5=_5c5==null?"":_5c5;
...
_77b[_77c]=_77e;
	...
	//dounion datagrid 获取添加的entity属性
	if(_5c5!=_77e){
	var entity=$(ed.target).data("entity");
	var flag=$(ed.target).hasClass("combobox-f")||$(ed.target).hasClass("combotree-f")||$(ed.target).hasClass("combogrid-f");
	if(_a.length==1)row[_a[0]]=_77e;
	if(_a.length==2){
	if(flag){
	row[_a[0]]=_77e==""?null:entity;
	}else{
	if(row[_a[0]]==null)row[_a[0]]={};
	row[_a[0]][_a[1]]=_77e;
	}
	}
	if(_a.length==3){
	if(row[_a[0]]==null)row[_a[0]]={};
	if(flag){
	row[_a[0]][_a[1]]=_77e==""?null:entity;
	}else{
	if(row[_a[0]][_a[1]]==null)row[_a[0]][_a[1]]={};
	row[_a[0]][_a[1]][_a[2]]=_77e;
	}
	}
	if(_a.length==4){
	if(row[_a[0]]==null)row[_a[0]]={};
	if(row[_a[0][_a[1]]]==null)row[_a[0]][_a[1]]={};
	if(flag){
	row[_a[0]][_a[1]][_a[2]]=_77e==""?null:entity;
	}else{
	if(row[_a[0]][_a[1]][_a[2]]==null)row[_a[0]][_a[1]][_a[2]]={};
	row[_a[0]][_a[1]][_a[2]][_a[3]]=_77e;
	}
	}
	_77a=true;
	_77b=row;
...
if(opts.pagination){
	//dounion datagrid 默认分页
	var o_o={};
	o_o[opts.entity+'.page.pageNo']=opts.pageNumber;
	o_o[opts.entity+'.page.row']=opts.pageSize;
	o_o[opts.entity+'.page.needCount']=true;
	$.extend(_7c4,o_o);
	//end
...
if(opts.sortName){
	//dounion datagrid 默认分页
	var o_o={};
	o_o[opts.entity+'.page.orderby']="t."+opts.sortName+" "+opts.sortOrder;
	$.extend(_7c4,o_o);
	//end
...
if(col){
	// dounion datagrid 点属性支持
	var _a=_83d.split(".");
	var _83e=_83b[_a[0]];
	for(var _k=1;_k<_a.length;_k++)if(_83e!=null)_83e=_83e[_a[_k]];
	// end
//var _83e=_83b[_83d];
...
col.formatter1=col.styler1=undefined;
	...
	//dounion datagrid pagination:true singleSelect:true idField:"id" sortName:"id",sortOrder:"desc"
	$.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{fit:false,sharedStyleSheet:false,frozenColumns:undefined,columns:undefined,fitColumns:false,resizeHandle:"right",autoRowHeight:true,toolbar:null,striped:false,method:"post",nowrap:true,idField:"id",url:null,data:null,loadMsg:"Processing, please wait ...",emptyMsg:"",rownumbers:false,singleSelect:true,ctrlSelect:false,selectOnCheck:true,checkOnSelect:true,pagination:true,pagePosition:"bottom",pageNumber:1,pageSize:10,pageList:[10,20,30,40,50],queryParams:{},sortName:"id",sortOrder:"desc",multiSort:false,remoteSort:true,showHeader:true,showFooter:false,scrollOnSelect:true,scrollbarSize:18,rownumberWidth:30,editorHeight:24,headerEvents:{mouseover:_6ae(true),mouseout:_6ae(false),click:_6b2,dblclick:_6b7,contextmenu:_6ba},rowEvents:{mouseover:_6bc(true),mouseout:_6bc(false),click:_6c3,dblclick:_6cd,contextmenu:_6d1},rowStyler:function(_86c,_86d){
	//end
...
//},onLoadSuccess:function(){
	//dounion datagrid onLoadSuccess
	},onLoadSuccess:function(data){
	var target=$(this);
	if($.data(this,"datagrid")==null)return;
	var opts = $.data(this,"datagrid").options;
	if(opts.singleSelect)return;
	for(var i=0;i<data.rows.length;i++){
	if(data.rows[i].ck=='checked'){
...
var row=find(_94b,_94c);
	//dounion treegrid _parentId parentId
if(row.parentId){
return find(_94b,row.parentId);
}else{
return null;
}
};
...
var prow=$.easyui.getArrayItem(_9b3.checkedRows,opts.idField,row.parentId);
...
return rr;
	...
	//dounion treegrid animate:true lines:true
	$.fn.treegrid.defaults=$.extend({},$.fn.datagrid.defaults,{treeField:null,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:true,animate:true,singleSelect:true,view:_99a,rowEvents:$.extend({},$.fn.datagrid.defaults.rowEvents,{mouseover:_910(true),mouseout:_910(false),click:_912}),loader:function(_9d8,_9d9,_9da){
	//end
var opts=$(this).treegrid("options");
if(!opts.url){
return false;
...
s=row[opts.textField];
	//dounion combobox 将值对应的entity赋给combobox控件
	$($.data(_a67,"combo").combo).prev().data("entity",row["entity"]);
	//end
_a6c=row;
var el=opts.finder.getEl(_a67,v);
if(!el.hasClass("combobox-item-selected")){
el.addClass("combobox-item-selected");
	//dounion combobox setValue时取消触发onSelect,放到点击事件中
...
return;
	...
	//dounion combobox 点击时触发onSelect
	else{
	opts.onSelect.call(_a88,row);
	}
	//end
if(opts.blurTimer){
...
$(_aa2).html(dd.join(""));
	...
	//dounion combobox valueField:"id" mode:"remote"
	$.fn.combobox.defaults=$.extend({},$.fn.combo.defaults,{valueField:"id",textField:"text",groupPosition:"static",groupField:null,groupFormatter:function(_aa5){
	//end
	return _aa5;
},mode:"remote",method:"post",url:null,data:null,queryParams:{},showItemIcon:false,limitToList:false,view:_aa0,keyHandler:{up:function(e){
nav(this,"prev");
e.preventDefault();
},down:function(e){
nav(this,"next");
...
if(node){
vv.push(node.id);
	//dounion combotree 设置entity
	$($.data(_ac3,"combo").combo).prev().data("entity",node.entity);
	//end
}
}
...
var vv=$.map(dg.datagrid("getSelections"),function(row){
	//dounion combogrid 添加entity
	$($.data(_ae3,"combo").combo).prev().data("entity",row);
	//end
...
return item?item[opts.textField]:undefined;
	...
	//dounion combogrid setValue时执行onSelect
	var rows = grid.datagrid("getRows");
	$.each(rows,function(_index,_item){
		var id_Field = $(_afe).combo("options");
		if($(_afe).combo("getValue") == _item.id){
			$($.data(_afe,"combo").combo).prev().data("entity",_item);
			opts.onSelect.call(_afe,_index,_item);
		}
	})
	//end
};
...
return $.extend({},$.fn.combo.parseOptions(_b22),$.fn.datagrid.parseOptions(_b22),$.parser.parseOptions(_b22,["idField","textField","mode"]));
	...
	//dounion combogrid id name remote
	$.fn.combogrid.defaults=$.extend({},$.fn.combo.defaults,$.fn.datagrid.defaults,{loadMsg:null,idField:"id",textField:"name",unselectedValues:[],mappingRows:[],mode:"remote",keyHandler:{up:function(e){
	//end
nav(this,"prev");
e.preventDefault();
},down:function(e){
...
var opts=_b9c.options;
	//dounion datebox 显示时分秒
	if(_b9a && _b99.className.indexOf('easyui-datebox')>=0 && _b9a.indexOf(":")>=0){
