var cvi_sztimer, cvi_szactive,videoList = [],lists= [],video = true,winWidth = $(window).width(),winHeight = $(window).height(),times = 0.254,times1 = 1,oldX, cvi_szimage = null, shiftzoom = {
	_shiftzoom : null,
	version : 3.2,
	released : '2009-09-16 16:11:00',
	defaultFading : true,
	defaultButtons : true,
	...
	defaultInfoblock : '',//'<div  style="text-align:right; font-size:12px">岐山地图<br >亮亮制作<br >支持键盘<\/div>',
	defaultOvborder : '',
	defaultCurpath : '',
	defaultZoom : 0,
	defaultXpos : 50,
	...
	add : function(ele, opts) {
		...
		if (!ele.active) {
			...
			if (document.images && document.createElement
				...
				var st, over, view, div = ele.parentNode, img = shiftzoom
						.E('div'), xref = shiftzoom.E('img'),granary = shiftzoom.E('div'),granaryView = shiftzoom.E('div'),videoView = shiftzoom.E('div'),
						gView = shiftzoom.E('div'),aView = shiftzoom.E('a'), gView0 = shiftzoom.E('div'), outer = shiftzoom.E('div');
				img.xid = (ele.id != "" ? ele.id : ele.id = uniqueID());
				div.appendChild(outer);
				outer.id = img.xid + '_wrap';
				outer.appendChild(xref);
				outer.appendChild(granary);
				...
				if (ele.naturalWidth && ele.naturalHeight) {
					...
					img.xfactor = 3;
					img.yfactor = 3;
					img.maxwidth = ele.width * 3;
					img.maxheight = ele.height * 3;
					tmp = null;
					delete tmp;
				}
				if (ele.width >= 100 && ele.width < img.maxwidth
						&& ele.height >= 100 && ele.height < img.maxheight) {
					img.fading = (typeof opts['fading'] === 'boolean'
							? opts['fading']
							: shiftzoom.defaultFading);
					img.buttons = (typeof opts['buttons'] === 'boolean'
							? opts['buttons']
							: shiftzoom.defaultButtons);
					img.nozoom = (typeof opts['nozoom'] === 'boolean'
							? opts['nozoom']
							: shiftzoom.defaultNozoom);
					img.bicubic = (typeof opts['bicubic'] === 'boolean'
							? opts['bicubic']
							: shiftzoom.defaultBicubic);
					img.overview = (typeof opts['overview'] === 'boolean'
							? opts['overview']
							: shiftzoom.defaultOverview);
					img.showcoords = (typeof opts['showcoords'] === 'boolean'
							? opts['showcoords']
							: shiftzoom.defaultShowcoords);
					img.pixelcoords = (typeof opts['pixelcoords'] === 'boolean'
							? opts['pixelcoords']
							: shiftzoom.defaultPixelcoords);
					img.percentcoords = (typeof opts['percentcoords'] === 'boolean'
							? opts['percentcoords']
							: shiftzoom.defaultPercentcoords);
					img.lowres = (typeof opts['lowres'] === 'string'
							? opts['lowres']
							: img.opts['lowres']);
					img.icons = (typeof opts['icons'] === 'object'
							? opts['icons']
							: img.opts['icons']);
					img.bicubic = (img.bicubic ? "bicubic" : "nearest-neighbor");
					img.curpath = (typeof opts['curpath'] === 'string'
							? opts['curpath']
							: img.opts['curpath']);
					img.intitle = (typeof opts['intitle'] === 'string'
							? opts['intitle']
							: img.opts['intitle']);
					img.outtitle = (typeof opts['outtitle'] === 'string'
							? opts['outtitle']
							: img.opts['outtitle']);
					img.infoblock = (typeof opts['infoblock'] === 'string'
							? opts['infoblock']
							: img.opts['infoblock']);
					img.defblock = img.infoblock;
					img.ovacolor = (typeof opts['ovacolor'] === 'string'
							? opts['ovacolor']
							: img.opts['ovacolor']);
					img.ovbcolor = (typeof opts['ovbcolor'] === 'string'
							? opts['ovbcolor']
							: img.opts['ovbcolor']);
					img.ovsfact = (typeof opts['ovsfact'] === 'number'
							? parseInt(Math.min(Math.max(10, opts['ovsfact']),
									50))
							: img.opts['ovsfact'])
							/ 100;
					img.millisec = (typeof opts['millisec'] === 'number'
							? parseInt(Math.min(Math.max(5, opts['millisec']),
									100))
							: img.opts['millisec']);
					img.ovaopac = (typeof opts['ovaopac'] === 'number'
							? parseInt(Math.min(Math.max(0, opts['ovaopac']),
									100))
							: img.opts['ovaopac']);
					img.opacity = (typeof opts['opacity'] === 'number'
							? parseInt(Math.min(Math.max(0, opts['opacity']),
									100))
							: img.opts['opacity']);
					img.ovborder = (typeof opts['ovborder'] === 'number'
							? parseInt(Math.min(Math.max(0, opts['ovborder']),
									20))
							: Math.min(Math.round(ele.width / 100), Math
											.round(ele.height / 100)));
					img.zoom = (typeof opts['zoom'] === 'number'
							? parseFloat(Math.min(Math.max(0, opts['zoom']),
									100))
							: img.opts['zoom']);
					img.xpos = (typeof opts['xpos'] === 'number'
							? parseFloat(Math.min(Math.max(0, opts['xpos']),
									100))
							: img.opts['xpos']);
					img.ypos = (typeof opts['ypos'] === 'number'
							? parseFloat(Math.min(Math.max(0, opts['ypos']),
									100))
							: img.opts['ypos']);
					img.opts = null;
					defopts = null;
					...
					st.position = (st.position == 'static' || st.position == ''
							? 'relative'
							: st.position);
					...
					img.bmode = (img.trident
							&& (document.compatMode == 'BackCompat' || document.compatMode == 'QuirksMode')
							? true
							: false);
					img.active = true;
					
					granary.id = img.xid +'_listDiv';
					granary.className = 'camera-number-big';
					st = granary.style;
					st.MozUserSelect = "none";
					st.KhtmlUserSelect = "none";
					...
					st.width = ele.width + 'px';
					st.height = ele.height + 'px';
					v = Math.max(0, Math.min(100, parseFloat(img.zoom)));
					f = v > 0 ? v / 100 : 0;
					xs = Math.round(Math.round(f * (img.maxwidth - img.minwidth))/img.minwidth*100)/100;
					ys = Math.round(Math.round(f * (img.maxheight - img.minheight))/img.minheight*100)/100;
					mw = Math.round(f * (img.maxwidth - img.minwidth))
					+ img.minwidth;
					mh = Math.round(f * (img.maxheight - img.minheight))
					+ img.minheight;
					...
					if (videoList.length>0) {
						for(var i in videoList) {
							var row = videoList[i];
							videoView = shiftzoom.E('div');
							if (row.type) {
								if(row.type.value == "90033-10"){
									videoView.className = 'camera-box ocamera-box camera2-s';
								}else if(row.type.value == "90033-20"){
									videoView.className = 'camera-box scamera-box camera-s';
								}
							} else {
								videoView.className = 'camera-box ocamera-box camera-s';
							}
							
							id = row.id;
							videoView.id = "video_div_"+id;
							st = videoView.style;
							st.left = (xs+1)*row.xAxis + 'px';
							st.top = (ys+1)*row.yAxis + 'px';
							st.zIndex = 3;
							
							aView = shiftzoom.E('a');
							var src = xref.src;
							aView.id = 'video_'+id;
							aView.className = 'camera-new';
							aView.href = 'javaScript:viewVideo('+row.id+','+row.bizId+',\'90031-10\');';
							
							var sView = shiftzoom.E('span');
							sView.innerHTML = row.sourceName;
							
							aView.appendChild(sView);
							videoView.appendChild(aView);
							granary.appendChild(videoView);
						}
					}
					if (lists.length>0) {
						$.each(lists,function(val){
							var row = lists[val],id = "";
							granaryView = shiftzoom.E('div');
							if (row.id) {
								id = row.id;
							} else {
								id = row.name+"".replace('号仓','')+"empty";
							}
							//设置id
							granaryView.id = 'warehouse_'+id;
							granaryView.className = 'silo-state';
							st = granaryView.style;
							st.left = (xs+1)*row.xAxis + 'px';
							st.top = (ys+1)* (row.yAxis) + 'px';
							st.zIndex = 2;
							granary.appendChild(granaryView);
							gView = shiftzoom.E('div');
							gView.className = 'warehouse-item';
							granaryView.appendChild(gView);
							aView = shiftzoom.E('a');
							aView.id = 'granary_'+id;
							//aView.href="javascript:showRightDiv("+id+","+parseInt(row.canClick)+");"
							
							if (parseInt(row.canClick)) {
								aView.href = 'javascript:granary('+id+',"'+row.name+'");';
								/*if(row.name.indexOf('油罐') != -1){
									aView.href = 'javaScript:showRightDiv('+id+',2);';
								} else {
									aView.href = 'javaScript:showRightDiv('+id+',1);';
								}*/
							}
							var aWidth = 0;
							gView.appendChild(aView);
							view = shiftzoom.E('i');
							var names = row.name+"";
							view.innerHTML = names.replace('号仓','').replace('库','').replace('油罐','');
							view.className = 'number';
						    aView.appendChild(view);
						    aWidth += view.clientWidth;
							//初始化显示内容
						    gView0= shiftzoom.E('div');
							if(row.qty && row.qty != '空仓'){
								gView0.className = 'con';
								if (!parseInt(row.canClick)) {
									gView0.style.display = 'none';
								}
								view=shiftzoom.E('p');
								var sView=shiftzoom.E('span')
								sView.className='name';
								if (typeof(row.categoryName) != "undefined" ) {
						    		sView.innerHTML = (!row.categoryName ? '' : row.categoryName);
						    	} else {
						    		sView.innerHTML = '&nbsp;';
						    	}
								if (row.name.indexOf('油罐') != -1) {
								...
								view.appendChild(sView);
//								if(row.grainTypeName && row.grainTypeName != ""){
//									var sgView=shiftzoom.E("span")
//									sgView.className='stock';
//									sgView.innerHTML = (row.grainTypeName+(row.years==null||row.years==""?"":(row.years+"年")));
//									view.appendChild(sgView);
//								}
								var emView =shiftzoom.E("em")
								if (row.weights && row.weights !='') {
									emView.innerHTML= parseInt(row.weights)+"吨";
								} else {
									emView.innerHTML= "-";
								}
								sView.appendChild(emView);
								gView0.appendChild(view);
							} else {
								gView0.className = 'con w66';
								var sView=shiftzoom.E('span');
								sView.className='tl';
								gView0.appendChild(sView);
								var xref0 = shiftzoom.E('img');
								xref0.src = (Server.staticPath+'/easyui-dounion/images/newhome/i_kc.png');
								xref0.className='vm';
								sView.appendChild(xref0);
								sView.innerHTML=sView.innerHTML+"空仓";
								gView0.appendChild(sView);
							}
//							view=shiftzoom.E('p');
//							sView=shiftzoom.E('span')
//							sView.className='weight';
//							var emView =shiftzoom.E("em")
//							if (row.volume) {
//								 emView.innerHTML=row.volume.substr(0,row.volume.indexOf('.'))
//							} else {
//							 	 emView.innerHTML= "-";
//							}
//							emView.innerHTML= "-";
...
//							var iView =shiftzoom.E("i")
//							iView.id='showTemp'+id;
//							iView.className='temp';
//							iView.title="粮温";
//							iView.innerHTML ='222-';
//							if(row.avgTemp){
//						    	iView.innerHTML =row.avgTemp;
//							    var sub = shiftzoom.E('sub');
//						    	sub.innerHTML = '℃';
//						    	iView.appendChild(sub);
//						    }
...
//							gView0.appendChild(view);
						  ...
						  /* 
						    if(row.autoStatus){
							   view.className = 'icon-state icon-state-spot';
							}
							*/
...
//								view = shiftzoom.E('s');
//								view.className = 'icon-state icon-state-spot';
//								if(row.isLocale == null || row.isLocale==1){
//									view.className = 'icon-state icon-state-abno';
//									//$("#granary_"+subId).find("s").removeClass().addClass("icon-state icon-state-abno");//.css( {"top":"-1%","right":"-1%","z-index":"5"} );
//								}else {
//									view.className = 'icon-state icon-state-spot';
//									//$("#granary_"+subId).find("s").removeClass().addClass("icon-state icon-state-spot");//.css( {"top":"-1%","right":"-1%","z-index":"5"});
//								}
//								 gView0.appendChild(view);
//							}
						   
						    aWidth += view.clientWidth;
							aView.appendChild(gView0);
						    aWidth += view.clientWidth;
						    if (aWidth && aWidth < 60 ) {
						    	aWidth = 70;
						    }
						    aView.style.width = (aWidth ? aWidth : 70)+'px';
						    if (row.showGasPh3Wran) {
								var it = shiftzoom.E('i');
							    it.className = 'icon-toxic';
							    it.id = 'grainPh3'+id;
							    gView.appendChild(it);
							}
						   
						    /*if (row.videoFlag == 'true') {
						    	aView = shiftzoom.E('a');
						    	aView.id = 'video_view_'+id;
						    	aView.name = 'video';
						    	if (video) {
							    	aView.style.display = 'none';
						    	}
						    	aView.style.color = 'red';
						    	aView.className = 'video-number';
						    	aView.innerHTML = '视频预览'
						    	gView.appendChild(aView);
						    }*/
						});
					}
					...
					st.left = '0px';
					st.top = '0px';
					st.boxShadow = "0px 0px 8px black";
					st.MozBoxShadow = "0px 0px 8px black";
					st.KhtmlBoxShadow = "0px 0px 8px black";
					st.WebkitBoxShadow = "0px 0px 8px black";
					div.appendChild(over);
					img.ctrlid = over.id;
					view = shiftzoom.E('div');
					st = view.style;
					...
					if (img.trident) {
						view.onmouseover = new Function('shiftzoom._showTooltip("'
								+ img.id + '");');
						view.onmouseout = new Function('shiftzoom._killTooltip("'
								+ img.id + '");');
					} else {
						view.setAttribute("onmouseover",
								"shiftzoom._showTooltip('" + img.id + "');");
						view.setAttribute("onmouseout",
								"shiftzoom._killTooltip('" + img.id + "');");
					}
					over.appendChild(view);
					img.kbinid = view.id;
					view = shiftzoom.E('div');
					...
					st.top = '0px';
					if (img.trident) {
						view.onclick = new Function('shiftzoom._setCursor(this,1,"'
								+ img.id + '");');
					} else {
						view.setAttribute("onclick",
								"shiftzoom._setCursor(this,1,'" + img.id
										+ "');");
					}
					over.appendChild(view);
					img.zoinid = view.id;
					...
					st.top = '0px';
					if (img.trident) {
						view.onclick = new Function('shiftzoom._setCursor(this,0,"'
								+ img.id + '");');
					} else {
						view.setAttribute("onclick",
								"shiftzoom._setCursor(this,0,'" + img.id
										+ "');");
					}
					over.appendChild(view);
					img.zoutid = view.id;
					...
					view.innerHTML = "100 / " + parseInt(img.xfactor * 100)
							+ " %";
					img.overid = 'oImg_over';
					img.viewid = 'oImg_view';
					img.tumbid = 'oImg_tumb';
					...
					if (img.showcoords) {
						img.onmousemove = (img.pixelcoords
								? shiftzoom._showCoords
								: img.percentcoords
										? shiftzoom._showPercent
										: shiftzoom._showLatLon);
					}
					...
					if (img.zoom > 0) {
						...
						shiftzoom._initZoom(1,1.2,'true','oImg');
						shiftzoom._initZoom(0,1.2,'true','oImg');
					}
		...
		if (img && typeof(v) === "object" && typeof(img.ctrlid) === "string") {
			...
			for (i = 0; i < v.length; i++) {
				...
				if (!ele && w >= 8 && h >= 8 && s != '') {
					...
					oe.unselectable = "on";
					oe.style.border = "0px none";
					oe.style.fontSize = "0px";
					oe.style.lineHeight = "0px";
					oe.style.margin = "0px";
					oe.style.padding = "0px";
					oe.style.textDecoration = "none";
					oe.style.mozUserSelect = "none";
					oe.style.khtmlUserSelect = "none";
					oe.style.webkitUserSelect = "none";
					img.appendChild(oe);
					ie = shiftzoom.E('img');
					if (img.notrans) {
						ie.src = img.curpath + "nop.gif";
						ie.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
								+ s + "', sizingMethod='scale')";
					} else {
						ie.src = s;
					}
					ie.width = w;
					...
					ie.style.margin = "0px";
					ie.style.padding = "0px";
					ie.style.border = "0px none";
					ie.style.width = q ? w + 'px' : (w / (img.maxwidth / 100))
							+ '%';
					ie.style.height = q
							? h + 'px'
							: (h / (img.maxheight / 100)) + '%';
					...
					m = (img.maxwidth / 100);
					if (q) {
						ie.style.top = (y ? y / n : 0) + '%';
						ie.style.left = (x ? x / m : 0) + '%';
					} else {
						if (!p || p == 4 || p == 5 || p == 6) {
							ie.style.top = ((y ? y / n : 0) - (parseFloat(ie.style.height) / 2))
									+ '%';
						} else if (p == 7 || p == 8 || p == 9) {
							ie.style.top = ((y ? y / n : 0) - parseFloat(ie.style.height))
									+ '%';
						} else if (p == 1 || p == 2 || p == 3) {
							ie.style.top = (y ? y / n : 0) + '%';
						}
						if (!p || p == 2 || p == 5 || p == 8) {
							ie.style.left = ((x ? x / m : 0) - (parseFloat(ie.style.width) / 2))
									+ '%';
						} else if (p == 3 || p == 6 || p == 9) {
							ie.style.left = ((x ? x / m : 0) - parseFloat(ie.style.width))
									+ '%';
						} else if (p == 1 || p == 4 || p == 7) {
							ie.style.left = (x ? x / m : 0) + '%';
						}
					}
					ie.style.mozUserSelect = "none";
					ie.style.khtmlUserSelect = "none";
					ie.style.webkitUserSelect = "none";
					ie.style.msInterpolationMode = img.bicubic;
					if (z) {
						ie.first = s;
						ie.secnd = z;
						if (!img.trident) {
	...
	moveto : function(img, x, y) {
		if (img && typeof(img.ctrlid) === "string") {
			if (img.parentNode.width > img.minwidth
					|| img.parentNode.height > img.minheight) {
				function LFL(m, i, n) {
					var d = parseFloat(m);
					if (d > n) {
						d = n;
				...
				var f, h, v, q = parseFloat((img.parentNode.width - img.minwidth)
						/ (img.maxwidth - img.minwidth));
				if (typeof(x) == "string") {
					if (x.match(/^([+-])?\d*([\.])?\d*$/)) {
				...
				if (typeof(y) == "string") {
					if (y.match(/^([+-])?\d*([\.])?\d*$/)) {
						...
						y = ((img.maxheight - img.minheight) * q)
								* (parseFloat(y) / 100);
					} else {
						y = 0;
					}
				...
				if (img.overview) {
					...
					view.left = Math
							.round((Math
									.abs(parseInt(img.parentNode.style.left)) / (img.parentNode.width / img.minwidth))
									* (img.ovsfact-0.05))
							- (img.bmode ? 2 : 0) + 'px';
					view.top = Math
							.round((Math
									.abs(parseInt(img.parentNode.style.top)) / (img.parentNode.height / img.minheight))
									* img.ovsfact)
							- (img.bmode ? 2 : 0) + 'px';
				}
			}
		...
		if (img && typeof(img.ctrlid) === "string") {
			if (typeof(v) === "number") {
				...
				if (img.parentNode.width > img.minwidth
						|| img.parentNode.height > img.minheight) {
					if (img.trident) {
						img.style.cursor = "url('" + img.curpath
								+ "grab.cur'),move";
					} else {
	...
	kenburns : function(img, obj) {
		if (img && typeof(img.ctrlid) === "string") {
			...
			var io = false, x = obj[0], y = obj[1], z = obj[2], p = obj[3] || false, s = obj[4] || false, d = obj[5] || false, b = obj[6] || false, a = obj[7] || false;
			if (typeof(p) === "number") {
				if (p == 3) {
					io = true;
				} else if (p == 0) {
			...
			sw = parseInt(img.parentNode.style.width);
			sh = parseInt(img.parentNode.style.height);
			sx = parseInt(img.parentNode.style.left);
			sy = parseInt(img.parentNode.style.top);
			ew = z > -1 ? Math.round(fz * (img.maxwidth - img.minwidth))
					+ img.minwidth : sw;
			eh = z > -1 ? Math.round(fz * (img.maxheight - img.minheight))
					+ img.minheight : sh;
			wf = parseFloat((ew - img.minwidth) / (img.maxwidth - img.minwidth));
			hf = parseFloat((eh - img.minheight)
					/ (img.maxheight - img.minheight));
			...
			var mx, my, mw, mh, q = 0, c = 0, t = Math.max(5, s), k = 1 / t;
			img.timer = window.setInterval(function() {
				q = io ? ((-Math.cos((k * c) * Math.PI) / 2) + 0.5) || 0 : Math
						.pow((k * c), p)
						|| 0;
				...
				if (c > t) {
					...
					if (img.automode) {
						...
						window.setTimeout("shiftzoom._next(shiftzoom.G('"
										+ img.id + "'))", img.autowait + 1000);
					} else {
						if (b && typeof(b) === "string"
								&& eval('typeof ' + b) == "function") {
							window.setTimeout("window['"
	...
	lowsource : function(img, src) {
		if (img && typeof(img.ctrlid) === "string") {
			if (typeof(src) === "string" && !img.automode && !img.zoomin
					&& !img.zoomout) {
				var low = new Image();
				low.onload = function() {
					img.lowres = low.src;
				...
				switch (d.toLowerCase()) {
					...
					case 'showcoords' :
						if (v == false && img.showcoords == true) {
							img.onmousemove = null;
							shiftzoom.G(img.xycoid).style.visibility = "hidden";
						} else if (v == true && img.showcoords == false) {
							shiftzoom.G(img.xycoid).style.visibility = "visible";
						...
						break;
					case 'pixelcoords' :
						if (v == false && img.pixelcoords == true
								&& img.showcoords == true) {
							img.onmousemove = shiftzoom._showLatLon;
						} else if (v == true && img.pixelcoords == false
								&& img.showcoords == true) {
							img.onmousemove = shiftzoom._showCoords;
						}
						img.pixelcoords = v;
						break;
					case 'percentcoords' :
						if (v == false && img.percentcoords == true
								&& img.showcoords == true) {
							img.onmousemove = shiftzoom._showLatLon;
						} else if (v == true && img.percentcoords == false
								&& img.showcoords == true) {
					...
					case 'buttons' :
						...
						} else if (v == true && img.buttons == false) {
					...
					case 'zoomin' :
						if (!img.nozoom
								&& !img.buttons
								&& (parseInt(img.parentNode.style.width) < img.maxwidth || parseInt(img.parentNode.style.height) < img.maxheight)) {
							if (v == true) {
								img.zoomin = true;
								img.zoomout = false;
								if (img.gecko) {
					...
					case 'zoomout' :
						if (!img.nozoom
								&& !img.buttons
								&& (parseInt(img.parentNode.style.width) > img.minwidth || parseInt(img.parentNode.style.height) > img.minheight)) {
							if (v == true) {
								img.zoomout = true;
								img.zoomin = false;
								if (img.gecko) {
	...
	get : function(img, d) {
		if (img && d && typeof(img.ctrlid) === "string") {
			if (d.toLowerCase() == "maxzoomx") {
				...
				var q = parseFloat((img.parentNode.width - img.minwidth)
						/ (img.maxwidth - img.minwidth)), z = Math.min(Math
								.max(q * 100, 0), 100)
						|| 0;
				var x = Math
						.min(
								Math
										.max(
												((Math
														.abs(parseFloat(img.parentNode.style.left)) / ((img.maxwidth - img.minwidth) * q)) * 100),
												0), 100)
						|| 0;
				var y = Math
						.min(
								Math
										.max(
												((Math
														.abs(parseFloat(img.parentNode.style.top)) / ((img.maxheight - img.minheight) * q)) * 100),
												0), 100)
						|| 0;
				return {
					x : x,
	...
	_setOverview : function(img) {
		...
		if(view){
			view.style.width = (Math.round((img.ovsfact * img.minwidth)
					/ (img.parentNode.width / img.minwidth)) - (img.bmode ? 0 : 2)+2)
					+ 'px';
			view.style.height = (Math.round((img.ovsfact * img.minheight)
					/ (img.parentNode.height / img.minheight)) - (img.bmode ? 0 : 2))
					+ 'px';
			var width = view.style.width;
			var height = view.style.height;
			view.style.backgroundSize = ''+width+' '+height+'';
			view.style.left = Math
					.round((Math.abs(img.parentNode.left) / (img.parentNode.width / img.minwidth))
							* img.ovsfact)
					- (img.bmode ? 2 : 0) + 'px';
			view.style.top = Math
					.round((Math.abs(img.parentNode.top) / (img.parentNode.height / img.minheight))
							* img.ovsfact)
					- (img.bmode ? 2 : 0) + 'px';
			view.maxleft = (img.minwidth * (img.ovsfact)) - (img.bmode ? 0 : 2)
					- parseInt(view.style.width)+2;
			view.maxtop = (img.minheight * img.ovsfact) - (img.bmode ? 0 : 2)
					- parseInt(view.style.height)+3;
		}
		return false;
	},
	...
	_showCoords : function(e) {
		if (cvi_szactive != null) {
			var k, t, x, y, ex, ey, px = 0, py = 0, o = shiftzoom
					.G(cvi_szactive), w = o.parentNode.width, h = o.parentNode.height;
			e = e ? e : window.event;
			ex = e.clientX;
			ey = e.clientY;
			if (e.pageX || e.pageY) {
			...
			shiftzoom.G(o.cposid).innerHTML = '<span>x:' + x + ' y:' + y
					+ '<\/span>';
		}
		return false;
	},
	_showPercent : function(e) {
		if (cvi_szactive != null) {
			var k, t, x, y, z, ex, ey, px = 0, py = 0, na = !Number.prototype.toFixed
					? 0
					: 1, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width, h = o.parentNode.height;
			e = e ? e : window.event;
			ex = e.clientX;
			ey = e.clientY;
			if (e.pageX || e.pageY) {
			...
			k = shiftzoom._getMousePos(ex, ey, px, py);
			t = shiftzoom._findPosXY(o);
			x = parseFloat((Math.min(Math.max(k.ex + k.ox - t.x, 0.0), w) / w)
					* 100);
			x = na ? x.toFixed(2) : parseInt(x);
			y = parseFloat((Math.min(Math.max(k.ey + k.oy - t.y, 0.0), h) / h)
					* 100);
	...
	_showLatLon : function(e) {
		if (cvi_szactive != null) {
			function parseDMS(v, n) {
				...
				return Math.abs(d) + "°" + parseInt(m) + "'" + parseInt(s)
						+ "'' " + n;
			}
			var k, t, x, y, lat, lon, ex, ey, px = 0, py = 0, o = document
					.getElementById(cvi_szactive), w = o.parentNode.width, h = o.parentNode.height;
			e = e ? e : window.event;
			ex = e.clientX;
			ey = e.clientY;
			if (e.pageX || e.pageY) {
			...
			shiftzoom.G(o.cposid).innerHTML = '<span>Lat: ' + lat
					+ ' &bull; Lon: ' + lon + '<\/span>';
		}
		return false;
	},
	...
	_showTooltip : function(id) {
		var ison, over, view, img = shiftzoom.G(id);
		ison = shiftzoom.G(img.ttipid);
		if (!ison) {
			...
			over.style.left = (t.x + 10) + 'px';
			over.style.top = t.y + 'px';
			over.style.visibility = 'visible';
			over.style.border = 'solid 2px white';
			over.style.borderRadius = '6px';
			over.style.MozBorderRadius = '6px';
			over.style.KhtmlBorderRadius = '6px';
			over.style.WebkitBorderRadius = '6px';
			over.style.boxShadow = '0px 0px 8px black';
			over.style.MozBoxShadow = '0px 0px 8px black';
			over.style.WebkitBoxShadow = '0px 0px 8px black';
			over.style.KhtmlBoxShadow = '0px 0px 8px black';
			over.style.MozUserSelect = "none";
			over.style.KhtmlUserSelect = "none";
			over.unselectable = "on";
			...
			if (!img.trident) {
				...
				view.style.MozBorderRadius = '4px';
				view.style.KhtmlBorderRadius = '4px';
				view.style.WebkitBorderRadius = '4px';
				over.appendChild(view);
			}
			view = shiftzoom.E('div');
	...
	_zoomIn : function(id, ct, st, sw, ew, sh, eh, sx, ex, sy, ey, nz) {
		if (!nz) {
			...
			if (parseInt(obj.parentNode.style.width) < obj.maxwidth
					|| parseInt(obj.parentNode.style.height) < obj.maxheight) {
				clearInterval(cvi_sztimer);
				mw = Math.max(obj.minwidth, Math.min(obj.maxwidth, Math
										.round(ew * ct / st + sw)));
				mx = Math.round(ex * ct / st + sx);
				mh = Math.max(obj.minheight, Math.min(obj.maxheight, Math
										.round(eh * ct / st + sh)));
				...
				shiftzoom.G(obj.textid).innerHTML = parseInt((mw / obj.minwidth)
						* 100)
						+ " / " + parseInt(obj.xfactor * 100) + " %";
				
						
				shiftzoom.zoomChange(obj,mw,mh);
				cvi_sztimer = setInterval("shiftzoom._zoomIn('" + id + "',"
								+ ct + "," + st + "," + sw + "," + ew + ","
								+ sh + "," + eh + "," + sx + "," + ex + ","
								+ sy + "," + ey + "," + nz + ")", obj.millisec);
			} else {
				clearInterval(cvi_sztimer);
			}
		}
	...
	_zoomOut : function(id, rm, ct, st, sw, ew, sh, eh, sx, ex, sy, ey, nz) {
		if (!nz) {
			var mw, mh, mx, my, obj = shiftzoom.G(id);
			if (parseInt(obj.parentNode.style.width) > obj.minwidth
					|| parseInt(obj.parentNode.style.height) > obj.minheight) {
				clearInterval(cvi_sztimer);
				mw = Math.max(obj.minwidth, Math.min(obj.maxwidth, Math
										.round(ew * ct / st + sw)));
				mx = Math.round(ex * ct / st + sx);
				...
				shiftzoom.G(obj.textid).innerHTML = parseInt((mw / obj.minwidth)
						* 100)
						+ " / " + parseInt(obj.xfactor * 100) + " %";
				shiftzoom.zoomChange(obj,mw,mh);
				cvi_sztimer = setInterval("shiftzoom._zoomOut('" + id + "',"
								+ rm + "," + ct + "," + st + "," + sw + ","
								+ ew + "," + sh + "," + eh + "," + sx + ","
								+ ex + "," + sy + "," + ey + "," + nz + ")",
						obj.millisec);
	...
	//缩小
	_clickSmall : function (id,eId,nId) {
		var img = shiftzoom.G(id);
		if ((img.parentNode.width > img.minwidth)) {
			times1 =Math.round((times1 - 0.254)*1000)/1000;
			var butt, sw, ew, sh, eh, sx, ex, sy, ey, st;
			sw = (img.parentNode.width) * -1;
			ew = (img.parentNode.width - img.minwidth) * -1;
			sh = (img.parentNode.height) * -1;
			eh = (img.parentNode.height - img.minheight) * -1;
			sx = parseInt(img.parentNode.style.left);
			ex = sx * -1;
			sy = parseInt(img.parentNode.style.top);
			ey = sy * -1;
			st1 = Math.max(1, Math.round((img.parentNode.width / img.minwidth)
							* 1));
			...
			var ct = times;
			var st = 1;		
			/*mw = Math.max(img.minwidth, Math.min(img.maxwidth, Math
											.round(sw * ct / st - sw)));
			mx = Math.round(ex * times1 / st1 + sx);
			mh = Math.max(img.minheight, Math.min(img.maxheight, Math
									.round(sh * ct / st - sh)));
			my = Math.round(ey  * times1 / st1 + sy);*/
			mw = Math.max(img.minwidth, Math.min(img.maxwidth, Math
											.round(sw * ct / st - sw)));
			mx = Math.round(ex / st1 + sx);
			mh = Math.max(img.minheight, Math.min(img.maxheight, Math
									.round(sh * ct / st - sh)));
			my = Math.round(ey / st1 + sy);
			shiftzoom.G(img.infoid).style.visibility = 'visible';
			shiftzoom.G(img.textid).innerHTML = parseInt((mw / img.minwidth)
					* 100)
			...
			img.maxleft = img.parentNode.width - img.minwidth;
			img.maxtop = img.parentNode.height - img.minheight;
			shiftzoom.zoomChange(img,mw,mh);
			shiftzoom._setOverview(img);
			if (mw == winWidth && mh == winHeight) {
				img.style.cursor = "crosshair";
				if (img.overview) {
					shiftzoom.G(img.overid).style.visibility = "hidden";
				}
			...
			var narrow = $('#'+nId);
			if (mw <= img.minwidth){
				narrow.removeAttr('href');
				narrow.addClass('not-narrow');
			} else {
				big.removeAttr('href').attr('href','javascript:shiftzoom._clickBig(\'oImg\',\'enlarge\',\'narrow\');');
				big.removeClass('not-enlarge');
				narrow.removeAttr('href').attr('href','javascript:shiftzoom._clickSmall(\'oImg\',\'enlarge\',\'narrow\');');
				narrow.removeClass('not-narrow');
			}
			if (img.divbug) {
				img.parentNode.firstChild.style.width = mw + 'px';
				img.parentNode.firstChild.style.height = mh + 'px';
	...
	//放大
	_clickBig : function (id,eId,nId) {
		var img = shiftzoom.G(id);
		if ((img.parentNode.width < img.maxwidth)) {
			times1 =Math.round((times1 + 0.254)*1000)/1000;
			var butt, sw, ew, sh, eh, sx, ex, sy, ey, st;
					...
					.max(
							...
							Math
									.min(
											ew,
											Math
													.round(((winWidth*0.5 - sx) * (img.maxwidth / img.parentNode.width))
															- (img.minwidth * 0.5)
															+ sx)))
					* -1;
			ey = Math
					.max(
							...
							Math
									.min(
											eh,
											Math
													.round(((winHeight*0.5 - sy) * (img.maxheight / img.parentNode.height))
															- (img.minheight * 0.5)
															+ sy)))
					* -1;
			st1 = Math.max(1, Math.round((img.maxwidth / img.parentNode.width)
							* 2));
			var ct = times;
			var st = 1;		
			img.visibility = 'visible';
	...
	zoomChange : function(img,mw,mh) {
		var mx = Math.round(mw / img.minwidth*100)/100;
		var my = Math.round(mh / img.minheight*100)/100;
		var wRatio = Math.round(mw/imgWidth*10000)/10000;
 		var hRatio = Math.round(mh/imgHeight*10000)/10000;
 		var ratio = wRatio> hRatio ? wRatio : hRatio;
		//摄像头
		if (videoList.length>0) {
			for(var i in videoList) {
				var row = videoList[i],id = row.id;
				warehouse = shiftzoom.G('video_div_'+id);
				if (warehouse) {
					styles = warehouse.style;
					styles.left = Math.round(mx * row.xAxis *100)/100+'px';
					styles.top = Math.round(my * (row.yAxis+divHeight) * 100)/100 - divHeight +'px';
					var classNames = "camera";
					if(row.type && row.type.value == "90033-10"){
						classNames = "camera2";
					}
					warehouse.className = 'camera-box ocamera-box';
//					if (mx <1.75) {
//						//小
//					//	//console2290 m<1.75")
//						//warehouse.className = 'camera-box '+classNames+'-s';
//						warehouse.className = 'camera-box ocamera-box';
//					} else if (mx < 3) {
//					//	//console2294 m<3")
//						//warehouse.className = 'camera-box '+classNames+'-m';
//						warehouse.className = 'camera-box ocamera-box';
//					} else {
//					//	//console2298 else")
//						warehouse.className = 'camera-box ocamera-box';
//						//warehouse.className = 'camera-box '+classNames+'-l';
//					}
				}
			}
		}
		$.each(lists,function(val){
			...
			warehouse = shiftzoom.G('warehouse_'+id);
			if (warehouse) {
				style = warehouse.style;
				warehouse.className = 'silo-state';
//				if (mx <1.75) {
//					//console2320 m<1.75")
//						warehouse.className = 'silo-state';
//					if (mx <1.3 && winWidth<=1366) {
//						//console2325 mx <1.3 && winWidth<=1366")
//						warehouse.className = 'silo-state';
//					} else {
//						//console2329  else")
//						//中号的时候,将吨舍去小数位
//						warehouse.className = 'silo-state silo-state-m';
//					}
//				} else if (mx < 3) {
//					//console2335  m<3")
//					warehouse.className = 'silo-state silo-state-l';
//				} else {
//					//console2338  else")
//					warehouse.className = 'silo-state silo-state-l';
//				}
//				if(mx < 1.75){
//					style.left = Math.round((parseInt(row.x))*ratio)-(divHeight/2)+'px';
//					style.top = Math.round((parseInt(row.y))*ratio)-(divHeight+3)*2+'px';
//				} else {
//					style.left = Math.round((parseInt(row.x))*ratio)-(divHeight/2)+'px';
//					style.top = Math.round((parseInt(row.y))*ratio)-(divHeight+3)+'px';
//				}
				style.left = Math.round((parseInt(row.x))*ratio)-(divHeight/2)+'px';
				style.top = Math.round((parseInt(row.y))*ratio)-divHeight+3+'px';
			}
	...
	_stopZoom : function() {
		...
		img.zoomin = false;
		img.zoomout = false;
		img.parentNode.left = parseInt(img.parentNode.style.left);
		img.parentNode.top = parseInt(img.parentNode.style.top);
		img.parentNode.width = parseInt(img.parentNode.style.width);
		img.parentNode.height = parseInt(img.parentNode.style.height);
		img.maxleft = img.parentNode.width - img.minwidth;
		img.maxtop = img.parentNode.height - img.minheight;
		if (img.parentNode.width > img.minwidth
				|| img.parentNode.height > img.minheight) {
		...
		shiftzoom.G(img.textid).innerHTML = parseInt((img.parentNode.width / img.minwidth)
				* 100)
				+ " / " + parseInt(img.xfactor * 100) + " %";
		if (img.lowres && img.highres) {
			shiftzoom.source(img, img.highres, false, true);
		}
		cvi_sztimer = setInterval("shiftzoom._fadeInfo('" + img.id + "',100)",
				30);
	...
	_catchWheel : function(e) {
		...
		if (cvi_szactive != null && v != 0) {
			cvi_szimage = true;
			shiftzoom._initZoom(d, (d == 1 ? 2 : 1), true);
		}
		return false;
	},
	...
	_catchKey : function(e) {
		...
		var rm = false, mm = false, k, t, ex, ey, px = 0, py = 0, obj = shiftzoom
				.G(img.infoid).style;
		e = e ? e : window.event;
		if (e.which) {
			mm = (e.which == 2);
		...
		if (((e.altKey && !e.shiftKey) || rm || img.zoomout)
			...
			sy = parseInt(img.parentNode.style.top);
			ey = sy * -1;
			st = Math.max(1, Math.round((img.parentNode.width / img.minwidth)
							* 3));
			...
			cvi_sztimer = setInterval("shiftzoom._zoomOut('" + img.id + "',"
							...
							+ "," + sw + "," + ew + "," + sh + "," + eh + ","
							+ sx + "," + ex + "," + sy + "," + ey + ","
							+ img.nozoom + ")", img.millisec);
		} else if (((!e.altKey && e.shiftKey) || mm || img.zoomin)
				&& !img.automode
				&& !img.nozoom
			...
			cvi_sztimer = setInterval("shiftzoom._zoomIn('" + img.id + "',0,"
							+ st + "," + sw + "," + ew + "," + sh + "," + eh
							+ "," + sx + "," + ex + "," + sy + "," + ey + ","
							+ img.nozoom + ")", img.millisec);
		} else if (img.parentNode.width > img.minwidth
				|| img.parentNode.height > img.minheight) {
			if (img.automode) {
				shiftzoom.stop(img);
			}
			...
			var x = parseInt(img.parentNode.style.left), y = parseInt(img.parentNode.style.top);
	...
	_initZoom : function(d, v, w, s) {
		var img;
		if (s) {
			img = shiftzoom.G(s);
			cvi_szactive = s;
		} else {
			 img = shiftzoom.G(cvi_szactive);
		}
		var sw, ew, sh, eh, sx, ex, sy, ey, st;
		if (img.automode) {
			shiftzoom.stop(img);
		}
		if (d == 0
			...
			sy = parseInt(img.parentNode.style.top);
			ey = sy * -1;
			var vs = Math.round((img.parentNode.width / img.minwidth)*100)/100;
			if (vs == 1) {
				st = 1;
			} else {
				st = Math.max(1, Math.round((vs* v)*100)/100);
			}
					...
					.max(
							...
							Math
									.min(
											ew,
											Math
													.round((((img.minwidth / 2) - sx) * (img.maxwidth / img.parentNode.width))
															- (img.minwidth * 0.5)
															+ sx)))
					* -1;
			ey = Math
					.max(
							...
							Math
									.min(
											eh,
											Math
													.round((((img.minheight / 2) - sy) * (img.maxheight / img.parentNode.height))
															- (img.minheight * 0.5)
															+ sy)))
					* -1;
			var vs = Math.round((img.maxwidth / img.parentNode.width)*100)/100;
			if (vs == 1) {
				st = 1;
			} else {
				st = Math.max(1, Math.round((vs* v)*100)/100);
			...
			if (!img.automode && !img.zoomout && !img.zoomin) {
				function setoverview() {
					...
					if (img.parentNode.width > img.minwidth
						...
						if (img.overview) {
							shiftzoom._setOverview(img);
							if(shiftzoom.G(img.overid)){
								shiftzoom.G(img.overid).style.visibility = "visible";
							}
						}
					} else {
						img.style.cursor = "crosshair";
						if (img.overview && shiftzoom.G(img.overid)) {
							shiftzoom.G(img.overid).style.visibility = "hidden";
						}
					}
					img.pointer = img.style.cursor;
				...
				times1 =Math.round(((ct*0.254))*1000)/1000;
				if (d == 0
						&& (parseInt(img.parentNode.style.width) > img.minwidth || parseInt(img.parentNode.style.height) > img.minheight)) {
					mw = Math.max(img.minwidth, Math.min(img.maxwidth, Math
											.round(ew * ct / st + sw)));
	...
	_legend : function (mw,img) {
		var big = $('#enlarge');
		var narrow = $('#narrow');
		if (mw >= img.maxwidth ) {
			big.removeAttr('href');
			big.addClass('not-enlarge');
		} else if (mw <= img.minwidth){
			narrow.removeAttr('href');
			narrow.addClass('not-narrow');
		} else {
			big.removeAttr('href').attr('href','javascript:shiftzoom._initZoom(1,1.2,\'true\',\'oImg\');');
			big.removeClass('not-enlarge');
			narrow.removeAttr('href').attr('href','javascript:shiftzoom._initZoom(0,1.2,\'true\',\'oImg\');');
			narrow.removeClass('not-narrow');
		}
	},
	_panKey : function(h, v, s, a) {
		if (cvi_szactive != null) {
			...
			if (!img.automode
					&& (img.parentNode.width > img.minwidth || img.parentNode.height > img.minheight)) {
				var x = Math.max(0, Math.min(img.maxleft,
								Math.abs(parseInt(img.parentNode.style.left))
										- (s ? 4 * h : a ? h / 4 : h)));
				var y = Math.max(0, Math.min(img.maxtop,
								Math.abs(parseInt(img.parentNode.style.top))
										- (s ? 4 * v : a ? v / 4 : v)));
	...
	_fadeImage : function(id, o) {
		...
		if (o <= 100) {
			...
			o += 10;
			window.setTimeout("shiftzoom._fadeImage('" + id + "'," + o + ")",
					30);
		} else {
			if (img.buttons) {
				shiftzoom.G(img.ctrlid).style.visibility = 'visible';
	...
	_fadeInfo : function(id, o) {
		clearInterval(cvi_sztimer);
		var img = shiftzoom.G(id), obj = shiftzoom.G(img.infoid);
		if (o > 0 && cvi_szactive == img.id && !img.zoomin && !img.zoomout) {
			...
			o -= 5;
			cvi_sztimer = setInterval("shiftzoom._fadeInfo('" + id + "'," + o
							+ ")", 50);
		} else {
			if (img.trident) {
	...
	_stopPan : function() {
		...
		if (img.overview && shiftzoom.G(img.viewid)) {
			view = shiftzoom.G(img.viewid).style;
			view.left = Math
					.round((Math.abs(parseInt(img.parentNode.style.left)) / (img.parentNode.width / img.minwidth))
							* img.ovsfact)
					- (img.bmode ? 2 : 0) + 'px';
			var top = Math
					.round((Math.abs(parseInt(img.parentNode.style.top)) / (img.parentNode.height / img.minheight))
							* img.ovsfact)
					- (img.bmode ? 0 : -3);
			if (!parseInt(img.parentNode.style.top)) {
				top = 0;
			}
			view.top = top + 'px';
		}parseInt(img.parentNode.style.top)
		shiftzoom._shiftzoom = null;
		return false;
	},
	...
	_whileMove : function(e) {
		...
		if (view && cen[cen.length - 1] == 'view' && view.maxleft
				&& view.maxtop) {
			var l = Math.max(0, Math.min(view.maxleft, Math
									.abs(parseInt(view.style.left))
									+ (e.clientX - view.mouseX)));
			var t = Math.max(0, Math.min(view.maxtop, Math
									.abs(parseInt(view.style.top))
									+ (e.clientY - view.mouseY)));
			view.style.left = (l - (img.bmode ? 2 : 0)) + 'px';
			view.style.top = (t - (img.bmode ? 2 : 0)) + 'px';
			view.mouseX = e.clientX;
			view.mouseY = e.clientY;
			var x = Math.max(0, Math.min(img.maxleft, Math.abs(l
									* (img.parentNode.width / img.minwidth)
									* (1 / (img.ovsfact)))));
			var y = Math.max(0, Math.min(img.maxtop, Math.abs(t
									* (img.parentNode.height / img.minheight)
									* (1 / img.ovsfact))));
			img.parentNode.style.left = (x * -1) + 'px';
			img.parentNode.style.top = (y * -1) + 'px';
