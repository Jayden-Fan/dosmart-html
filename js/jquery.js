...
(function(E, B) {
	...
	c = function() {
		...
		b.extend({
			...
			inArray: function(j, s) {
				...
				for (var v = 0,
				z = s.length; v < z; v++) if (s[v] === j) return v;
				return - 1
			},
			merge: function(j, s) {
				var v = j.length,
	...
	c.fn.extend({
		...
		addClass: function(a) {
			...
			if (a && typeof a === "string") for (var b = (a || "").split(ha), d = 0, e = this.length; d < e; d++) {
				...
				if (f.nodeType === 1) if (f.className) {
					...
					l = f.className,
					k = 0,
					o = b.length; k < o; k++) if (h.indexOf(" " + b[k] + " ") < 0) l += " " + b[k];
		...
		toggleClass: function(a, b) {
			...
			return this.each(function() {
				if (d === "string") for (var f, h = 0,
				l = c(this), k = b, o = a.split(ha); f = o[h++];) {
					k = e ? k: !l.hasClass(f);
		...
		hasClass: function(a) {
			a = " " + a + " ";
			for (var b = 0,
			d = this.length; b < d; b++) if ((" " + this[b].className + " ").replace(sa, " ").indexOf(a) > -1) return true;
			return false
		},
		val: function(a) {
			if (!arguments.length) {
	...
	function() {
		...
		k.filter = function(g, i, n, m) {
			for (var p, q, u = g,
			y = [], F = i, M = i && i[0] && k.isXML(i[0]); g && i.length;) {
	...
	c.fn.extend({
		...
		has: function(a) {
			...
			return this.filter(function() {
				for (var d = 0,
				e = b.length; d < e; d++) if (c.contains(this, b[d])) return true
			})
		},
		not: function(a) {
	...
	c.fn.extend({
		...
		remove: function(a, b) {
			for (var d = 0,
			e; (e = this[d]) != null; d++) if (!a || c.filter(a, [e]).length) {
				if (!b && e.nodeType === 1) {
	...
	c.extend({
		...
		cleanData: function(a) {
			...
			l = 0,
			k; (k = a[l]) != null; l++) if (! (k.nodeName && c.noData[k.nodeName.toLowerCase()])) if (d = k[c.expando]) {
	...
	vb = /^(?:toggle|show|hide)$/,
	wb = /^([+\-]=)?([\d+.\-]+)(.*)$/,
	ba, pa = [["height", "marginTop", "marginBottom", "paddingTop", "paddingBottom"], ["width", "marginLeft", "marginRight", "paddingLeft", "paddingRight"], ["opacity"]];
	c.fn.extend({
		show: function(a, b, d) {
	...
	function(a) {
		...
		o = b.offsetLeft; (b = b.parentNode) && b !== l && b !== h;) {
